...
@section('content')
    ...
    <div class="album py-5 px-4 bg-body-tertiary">
        ...
        @endif
        @include('inc.message')
        <form method="POST" enctype="multipart/form-data" action="{{route('admin.users.update', $user)}}" class="p-4 p-md-5 border mx-5 rounded-3 bg-body-tertiary" >
            @csrf
            @method('PUT')
            <div class="container">
                <div class="mb-3">
                    ...
                    <input type="text" name="name" id="name" value="{{old('name')??$user->name}}" placeholder="Бриханкин Олег" class="form-control @error('name')is-invalid @enderror">
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="is_admin" id="is_admin1" value="0" @if(($user->is_admin == 0)) checked @endif>
                        <label class="form-check-label" for="is_admin1">
                            Пользователь
                        </label>
                    </div>
                    <div class="form-check">
                        <input class="form-check-input" type="radio" name="is_admin" id="is_admin2" value="1" @if(($user->is_admin == 1)) checked @endif>
                ...
                <div class="mb-3">
                    ...
                    <input type="email" name="email" id="email" value="{{old('email')??$user->email}}" placeholder="name@example.com" class="form-control @error('email')is-invalid @enderror">
                    @error('email')
                    <div id="validationServer03Feedback" class="invalid-feedback">
                        {{$message}}
                    </div>
                ...
                <button class="w-100 btn btn-lg btn-primary" type="submit">Редактировать данные</button>
            </div>
        </form>
    </div>
@endsection
